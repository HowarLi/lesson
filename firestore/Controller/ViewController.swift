...
class ViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        //註冊
        //order(by: "time", descending: true)是將新增順序進入
        let _ = presidentsCols.order(by: "time", descending: true).addSnapshotListener { (presidentsSnapshot:QuerySnapshot?, error:Error?) in
            //if error != nil如果等於nil就會跳出錯誤訊息print("error:\(error!.localizedDescription)");
            if error != nil {
                print("error:\(error!.localizedDescription)");
                
                //return是為了跳出整個methed或是func
                return;
            }
            
            //isEmpty這裡如果抓到Bool值如果是空的就會跳至self.navigationItem這行
...
extension ViewController{
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell{
        ...
        let queryDocumentSnapshot = queryDocuments[indexPath.row]
        //抓到裡面的資料queryDocumentSnapshot.data()
